...
**How a document is chunked:** not every 800 characters, but on the document's own structure. The text is split into headings, paragraphs, list items, table rows, fenced code blocks and sentences, and those pieces are packed into chunks of up to 1000 characters (380-590 on average, depending on the page) with a short whole-sentence overlap. A chunk therefore never ends mid-sentence, never mixes two sections, and starts with the heading path it came from (`# Page > Section`), so a retrieved fragment states what it is about. It is plain string processing: no extra embedding call, no extra Workers AI usage. Measured over three real documentation pages, chunks that started mid-sentence went from 13/16 to 0, words split across two chunks from 10 to 0 and chunks mixing two sections from 11 to 0, for 10% more indexed characters (`npm run test:unit` prints the table). Because chunks are smaller, `/ask` now retrieves 8 of them by default instead of 5, keeping the same amount of context in front of the model (`topK` in the request body still overrides it).

Ingestion endpoints are rate limited per IP, accept only public http(s) pages, and index at most 100 chunks (~50k characters) per request (`truncated: true` when a page is longer). Enable them with `wrangler secret put INGEST_TOKEN`; without the secret they answer 503.

### Large documents (Cloudflare Workflows)

For pages or texts beyond the 100-chunk cap, `POST /ingest-jobs` starts a durable **Workflow** and answers `202` with a job id. The job reads the page (URL jobs), then embeds and stores it in batches of 50 chunks; each batch is a step retried with exponential backoff, so a Workers AI or Vectorize error does not restart the whole document. Vector ids are derived from the job and chunk position, so a retried batch overwrites its own vectors instead of duplicating them. A job indexes up to 1000 chunks (~500k characters, the Jina Reader cap); text sent inline is limited to 900 KiB (Workflow payloads are capped at 1 MiB). Same token and rate limit as the other ingestion endpoints. Completed job status is kept for 3 days on the Workers Free plan.
